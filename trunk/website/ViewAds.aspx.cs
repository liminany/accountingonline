...
public partial class ViewAds : System.Web.UI.Page
{
    ...
    protected void btnSendMessage_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (nRetuenValue == 1)
            {
                ...
                ClientScript.RegisterStartupScript(this.Page.GetType(), "ScrollToControl", sGoToBottom);               
    ...
    protected void btnEditAds_Click(object sender, EventArgs e)
    {
        try
        {
            if (!string.IsNullOrEmpty(Page.RouteData.Values["AdsID"].ToString()))
            {
                Response.Redirect("../../EditAds?AdsID=" + Page.RouteData.Values["AdsID"].ToString() + "&Type=edit", false);
            }
        }
        catch (Exception ex)
        {
